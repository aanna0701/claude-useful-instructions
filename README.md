...
**Stage 1 — 컨텍스트 추출**
NotebookLM에 저장된 이력서/포트폴리오/논문에서 체계적으로 정보를 추출해 구조화된 컨텍스트 문서를 생성합니다. 생성된 문서는 NotebookLM에 다시 업로드되어 이후 단계에서 참조됩니다.

**Stage 2 — 경력 문서 생성**
컨텍스트를 바탕으로 두 가지 문서를 작성합니다:
- **상세 경력 기술서**: 시간순 프로젝트/업무 기술, 수치 성과 포함
- **인사 관점 에세이**: HR 담당자 시각의 소프트 스킬 서사

**Stage 3 — 자소서 작성 (Writer-Reviewer 루프)**
Writer 에이전트가 초안을 작성하고, Reviewer 에이전트가 7개 항목으로 평가해 피드백을 제공합니다. 기준을 충족할 때까지 반복합니다.
