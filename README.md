...
| 3 | 사실 검증 | NLM 직접 쿼리로 원본 문서와 대조, 수치/날짜 확인 |
...
NotebookLM을 단순 검색 DB가 아닌 **AI 합성 엔진**으로 활용합니다. 업로드된 모든 문서를 교차 분석해 추론하는 NLM의 기능을 각 Stage에서 적극 사용합니다.

- **Stage 1**: 3라운드 합성 쿼리 — 패턴 분석(Round 1) → 구조화 추출(Round 2) → 갭 분석(Round 3)
- **Stage 2**: 각 경력 항목 작성 전 NLM에 심층 쿼리로 구체성 확보
- **Stage 3 Writer**: 초안 전 JD 특화 NLM 합성 쿼리 (최적 경험 소재 선정)
- **Stage 3 Reviewer**: 사실 검증 시 NLM에 직접 쿼리해 원본 문서와 대조
