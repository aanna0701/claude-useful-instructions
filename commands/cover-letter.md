...
1. **경고 출력:** `⚠️ NotebookLM 연결이 끊어졌습니다 (토큰 만료 가능성). nlm login으로 재인증하면 품질이 올라갑니다.`
...
업로드 완료 후 사용자에게 반드시 확인: **"Stage 3를 새 채팅에서 시작할까요, 이어서 할까요?"**
...
각 항목 0-100 **연속 점수** (25/50/75/100 고정값 금지 — 예: 37, 68, 83처럼 실제 판단값 사용) → 총점 = 7개 평균.
